...
//////////////////////////////////////
		 //CONTENT START//
//////////////////////////////////////
	sortBy(sortable, byKey, order){
		let sortSfx = 1;
		if (order === "DESC"){
			sortSfx = -1;
		}
        sortable.sort(function(a, b) {
            return sortSfx * (a[byKey] - b[byKey]);
        });
        return sortable;
	},

    countNSort(arr){
		const stat = arr.reduce((prev, curr) => (prev[curr] = ++prev[curr] || 1, prev), {});

        let sortable = [];
        for (let i in stat) {
            sortable.push([i, stat[i]]);
        }
        sortable.sort(function(a, b) {
            return b[1] - a[1];
        });
	...
	translateTOPOS: function(intValue, type){
		if(type === "SP" || type === "WP"|| type === "WPM"){
            if(intValue < 1){
                return "00";
            }	else	{
            	return "99"
	...
	getSimpleDeviceType(deviceID){
		let type = deviceID.substr(0,2);
		if(type === "SA"){
			return deviceID.substr(0,4);
		}
		return type;
	},
	getByteFromStr(message, bytePos, length){
		if(length){

			return message.substring((bytePos - 1) * 2,(bytePos - 1 + length) * 2)
		}   else    {
			return message.substring((bytePos - 1) * 2, message.length - 2)
		}
