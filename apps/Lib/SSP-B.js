...
module.exports = {
    ...
    sspbConfigMFiChallengeGet: function(SSDeviceID, MFiChallenage){
	    var data = {
		    isRequest 	: true,
		    QoS 		    : 2,
		    QosNeeded	: 1,
    ...
    sspbDataSTPost: function(SSDeviceID){
        var data = {
            isRequest 	: true,
            QoS 		: 2,
            QosNeeded	: 1,
        ...
        data.payload = JSON.stringify(SSPBObjects.dataSTObject(SSDeviceID));
        data.Topic = createTopic(data.topicType, data.topicExt);
        var msg = constructMessage.constructMessage(data.isRequest,data.QoS,data.dup,data.MessageType,data.Topic,data.MessageID,data.MessageIDextended,data.payload,SSDeviceID);
        TCPClient.TCPSocketWrite(SSDeviceID, msg, data.topicType, data);
        return data;
    },
    sspbDeviceMalfunctionGet: function(SSDeviceID){
        var data = {
            isRequest 	: true,
            QoS 		: 2,
            QosNeeded	: 1,
    ...
    sspbQE: function(SSDeviceID, action, sepid, options){
        ...
        switch(action){
            ...
            case "DMM":
                ...
                if(!data.topicExt["duration"]){
	                data.topicExt["duration"] = 20;
                }
                break
            case "WP":
            case "WPM":
    ...
    sspbAlarm: function(deviceID, buzzState, ledState, duration){
        var data = {
            isRequest 	: true,
            QoS 		: 2,
            QosNeeded	: 1,
        ...
        let SSDeviceID = deviceID;
        if(deviceID.seraphType() === "SS"){
	        data.topicExt["sepid"] = deviceID
        }   else    {
	    ...
	    data.topicExt["buzz"] = parseInt(buzzState);
	    data.topicExt["led"] = parseInt(ledState);
	    data.topicExt["time"] = parseInt(duration);
        data.Topic = createTopic(data.topicType, data.topicExt);
        var msg = constructMessage.constructMessage(data.isRequest,data.QoS,data.dup,data.MessageType,data.Topic,data.MessageID,data.MessageIDextended,data.payload,SSDeviceID);
        TCPClient.TCPSocketWrite(SSDeviceID, msg, data.topicType, data);
        return data;
	...
	sspbAlarmHoldSM: function(SSDeviceID, holdTime){
		var data = {
			isRequest 	: true,
			QoS 		: 2,
			QosNeeded	: 1,
		...
		data.topicExt["time"] = parseInt(holdTime);
		data.Topic = createTopic(data.topicType, data.topicExt);
		var msg = constructMessage.constructMessage(data.isRequest,data.QoS,data.dup,data.MessageType,data.Topic,data.MessageID,data.MessageIDextended,data.payload,SSDeviceID);
		TCPClient.TCPSocketWrite(SSDeviceID, msg, data.topicType, data);
		return data;
	},
    sspbUpdateSS(SSDeviceID){
	    var data = {
		    isRequest 	: true,
		    QoS 		: 2,
		    QosNeeded	: 1,
