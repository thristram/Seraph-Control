...
HAPServer.prototype._handlePairStepTwo = function(request, response, session, objects) {
  ...
  var M2 = srpServer.computeM2();
    ...
    var pairSalt = bufferShim.from("MFi-Pair-Setup-Salt");
    var pairInfo = bufferShim.from("MFi-Pair-Setup-Info");

    var MFiChallenge = hkdf.HKDF("sha512", pairSalt, S_private, pairInfo, 32);
    var MFIChallengeData = crypto.createHash('sha1').update( MFiChallenge ).digest('hex');

	Seraph.devices.getDevice("SS88888888").MFiChallenge(MFIChallengeData, function(payload){
		console.log("GETTING MFI Challenge Data");
		var MFiProof = new Buffer(payload.MFiProof, "hex");
		var AccessoryCertificate = new Buffer(payload.AccessoryCertificate, "hex");

		var message = tlv.encode(
			HAPServer.Types.SIGNATURE, MFiProof,
			HAPServer.Types.CERTIFICATE, AccessoryCertificate
		);

		var S_key = srpServer.computeK();
		var encSalt = bufferShim.from("Pair-Setup-Encrypt-Salt");
		var encInfo = bufferShim.from("Pair-Setup-Encrypt-Info");
		var sessionKey = hkdf.HKDF("sha512", encSalt, S_key, encInfo, 32);

		var ciphertextBuffer = bufferShim.alloc(message.length);
		var macBuffer = bufferShim.alloc(16);
		encryption.encryptAndSeal(sessionKey, bufferShim.from("PS-Msg04"), message, null, ciphertextBuffer, macBuffer);
