...
var SLightController = {
	...
	getFirmwareVersion: function() {
		let SSDevice = Seraph.devices.getDevice(seraphConfig.SCdeviceID);
		if(SSDevice){
...
var SLCService = function() {
	...
	SLightController.setHKUUID(lightAccessory.UUID);
    ...
    lightAccessory
        ...
        .setCharacteristic(Characteristic.FirmwareRevision, SLightController.getFirmwareVersion())
    ...
    SSPLinker.HAPEvent.on('statusUpdate', function(deviceID, channel, value, ifReceipt){
        ...
        if(((receiptReserveTime > 0) && (ifReceipt)) || (receiptReserveTime <= 0)){
            if((deviceID === seraphConfig.deviceID) && (channel === seraphConfig.channelID)) {
	            ...
	            lightAccessory.updateReachability(SLightController.isOnline());
                setTimeout(function () {
                    reverseFlag = false;
                }, 1000)
            }
