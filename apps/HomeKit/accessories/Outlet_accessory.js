...
var SPowerController = {
	...
	getFirmwareVersion: function() {
		let SCDevice = Seraph.devices.getDevice(seraphConfig.SCdeviceID);
		if(SCDevice){
			return SCDevice.getFirmwareVersion();
		}
		return "1.0";
	},
	setHKUUID: function(UUID){
		let SEPDevice = Seraph.devices.getDeviceByMDID(seraphConfig.SCdeviceID, parseInt(seraphConfig.moduleID));
		if(SEPDevice){
			SEPDevice.getChannel(parseInt(seraphConfig.channelID)).homeKitUUID = UUID;
		}
	},
	isOnline: function(){
		let SEPDevice = Seraph.devices.getDeviceByMDID(seraphConfig.SCdeviceID, parseInt(seraphConfig.moduleID));
		if(SEPDevice){
			return (SEPDevice.deviceStatus.getStatus() === 1);
		}
		return false;
	}
};
...
var SPCService = function(){
	...
	SPowerController.setHKUUID(outlet.UUID);
    ...
    outlet
	    ...
	    .setCharacteristic(Characteristic.FirmwareRevision, SPowerController.getFirmwareVersion())
    ...
    SSPLinker.HAPEvent.on('statusUpdate', function(deviceID, channel, value, ifReceipt){
        ...
        if(((receiptReserveTime > 0) && (ifReceipt)) || (receiptReserveTime <= 0)){
            if((deviceID === seraphConfig.deviceID) && (channel === seraphConfig.channelID)){
                ...
                outlet.updateReachability(SPowerController.isOnline());
                reverseFlag = false;
            }
