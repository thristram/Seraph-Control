...
var Seraph_Sensor = {
    getCarbonMonoxideLevel: function() {
        ...
        if(SSDevice){
            value = SSDevice.getSensor("CO").getRealValue();
        }
    ...
    getCarbonMonoxidePeakLevel: function() {
        ...
        let SSDevice = Seraph.devices.getDevice(seraphConfig.deviceID);
        if(SSDevice){
            value = SSDevice.getSensor("CO").getPeakValue()
        }
        debug("Getting the current Carbon Monoxide Peak Level: " + value);
        return value;
    },
    getCarbonMonoxideDetected: function() {
        ...
        let SSDevice = Seraph.devices.getDevice(seraphConfig.deviceID);
        if(SSDevice){
            value = (SSDevice.getSensor("CO").sensorState.level > 1)
        }
        debug("Getting if current Carbon Monoxide is Detected: " + value);
        return value ? 1 : 0;
    },
	...
	setHKUUID: function(UUID){
		let SSDevice = Seraph.devices.getDevice(seraphConfig.deviceID);
		if(SSDevice){
			SSDevice.getSensor("CO").homeKitUUID = UUID;
		}
	}
};
