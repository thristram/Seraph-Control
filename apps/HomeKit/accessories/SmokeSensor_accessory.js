...
var Seraph_Sensor = {
    ...
    getSmokeDetected: function() {
        ...
        if(SSDevice){
            value = (SSDevice.getSensor("SM").getRealValue() > 0);
        }
        debug("Getting if Smoke is Detected: " + value ? "Yes" : "No" );
        return value ? 1 : 0;
    },
	...
	setHKUUID: function(UUID){
		let SSDevice = Seraph.devices.getDevice(seraphConfig.deviceID);
		if(SSDevice){
			SSDevice.getSensor("SM").homeKitUUID = UUID;
		}
	}
};
...
var SensorService = function() {
    ...
    SSPLinker.HAPEvent.on('smokeUpdate', function(deviceID){

        if(deviceID === seraphConfig.deviceID) {
            debug("Smoke Update on: " + deviceID );
            sensor
